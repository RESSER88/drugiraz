...
-- Obecnie każdy może czytać dane osobowe klientów - naprawa natychmiastowa
...
-- P1: Porządki w schematach - przeniesienie extensions z public do dedykowanego schema
CREATE SCHEMA IF NOT EXISTS extensions;

-- Tylko jeśli extension http jest w public (sprawdzenie bezpieczne)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'http' AND extnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')) THEN
        -- Przenosimy extension do dedykowanego schema
